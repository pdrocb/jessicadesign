...
- **El desarrollo local usa la rama Neon `dev`; `main` es producción.** El contexto local vive en el archivo ignorado `.neon` y `.env.local` debe reportar `NEON_BRANCH=dev`. Antes de ejecutar scripts que escriban datos o cambien esquema, confirmar esa variable; nunca hacer pruebas o migraciones contra `main`. Para restaurar el entorno aislado, usar `neon checkout dev` o `neon env pull --branch dev --file .env.local --service postgres --service auth`. Vercel conserva sus variables de producción y no se modifica al cambiar el entorno local. (Origen: aislamiento de datos de desarrollo, sep 2026.)
...
- **Las decisiones tipográficas explícitas de la clienta ganan sobre microoptimizaciones.** No retirar Karla Italic para ahorrar una sola variante de fuente: no es la causa del lag y forma parte de la pareja tipográfica solicitada. Priorizar primero imágenes sobredimensionadas y trabajo de layout por frame. (Origen: auditoría de performance, ago 2026.)
...
- **La pareja 6/6 de Morgan & Garrett e Isabelle & Marshall usa 5:4 en desktop y mobile.** Sus fuentes son horizontales (aprox. 5:4 y 3:2); forzarlas a 4:5 produce una fila excesivamente alta y recorta su composición. Tablet conserva 3:4 como excepción del mosaico intermedio. (Origen: polish de ritmo del Look Book Home, sep 2026.)
