...
import {
  saveLookBook,
  type LookBookSaveState,
} from "@/cms/content/look-book-actions";
import type { LookBookDocument } from "@/cms/content/look-book";

const initialState: LookBookSaveState = { status: "idle", message: "" };

export function LookBookPageEditor({ document }: { document: LookBookDocument }) {
  const [state, action, pending] = useActionState(saveLookBook, initialState);
  ...
  return (
    <form action={action} onChange={() => setDirty(true)} className="cms-editor-form">
      <CmsPageHeader
        ...
        actions={
          <>
            <a className="cms-secondary-link" href="/look-book" target="_blank" rel="noreferrer">
              View live page
            </a>
