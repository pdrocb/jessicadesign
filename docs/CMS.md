...
Los tokens neutrales viven en `cms/styles/tokens.css`; `controls.css` define la geometría y los estados compartidos. `CmsField` y `CmsButton` encapsulan el contrato accesible y visual usado por Home, Site Settings, Projects y Login. Cada error de campo se presenta junto al control, se enlaza mediante `aria-describedby`/`aria-errormessage` y conserva además un resumen anunciado; cuando un fallo no pertenece a un campo, el resumen recibe el foco y ofrece una ruta de reintento. `CmsPageHeader`, `CmsEditorChrome` y `CmsUnsavedChangesGuard` unifican la jerarquía de página, los estados `Unsaved / Saving / Saved / Error` y la protección al abandonar cambios. Esa protección permanece activa durante un guardado remoto y cubre enlaces, recargas, cierre de pestaña y navegación por historial. Los layouts específicos continúan en `app/admin/admin.css` hasta que exista repetición suficiente para extraer otra primitiva.
...
El índice de Look Book es una superficie operativa: desde cada fila se cambia el orden, `Published` y `Show on Home`. `Published` controla el archivo público completo; `Show on Home` filtra hasta siete proyectos publicados para el mosaico de Home. No existe un segundo orden: ambos destinos obedecen `cms_projects.position`, y mover una fila actualiza los dos. Los controles de orden se bloquean mientras esperan la respuesta y explican localmente cualquier fallo. Despublicar un proyecto también lo retira de Home. `New project` abre un flujo breve que exige título y primera portada, optimiza la fotografía antes de subirla y crea el proyecto como draft; permanece en una columna durante tablet y adopta la retícula ancha solo cuando existe espacio real. Abrir una fila lleva a `/admin/projects/[projectId]`, donde una barra superior persistente reúne `All projects`, `View live project`, `Cancel` y la acción primaria `Save project`. La eliminación permanente vive separada al final, dentro de una zona de peligro y siempre mediante una confirmación explícita.
...
Toda eliminación iniciada desde el CMS pasa por `CmsConfirmDialog`; no se usan diálogos del navegador. El componente nombra el objeto y la consecuencia, enfoca `Cancel` al abrir, conserva Escape y clic exterior, bloquea un segundo envío mientras una operación remota está pendiente y devuelve el foco al cerrar. Si el disparador desaparece junto con el elemento eliminado, el foco pasa explícitamente al elemento vecino. Testimonials y FAQs aclaran que el cambio permanece en el borrador hasta guardar Home; las fotografías y los proyectos aclaran que la eliminación en Neon es inmediata. Las futuras eliminaciones de galerías reutilizarán el mismo contrato.

Home presenta SEO como sección propia y el contenido en cuatro capítulos operativos —Opening, Services, Story & Proof y Closing— sin modificar el resto del documento persistido. Home, Site Settings y Project Editor conservan cambios fallidos, abren la sección que contiene el campo inválido y enfocan ese control para corregirlo. La validación nativa también intercepta el primer campo inválido antes de enviar, de modo que un control obligatorio dentro de una sección cerrada nunca bloquea el guardado en silencio. Los fallos esperables de Neon vuelven al mismo estado `Error` sin desmontar el formulario. Los enlaces internos interceptan una salida con cambios pendientes mediante una confirmación neutral; `beforeunload` cubre recargas y cierres de pestaña, y el historial restaura la ruta actual hasta que la usuaria confirme si desea salir.

Inquiries carga hasta 100 registros recientes y filtra en cliente por nombre, email, teléfono, celebración o venue. En desktop y tablet ancha conserva la lista y el detalle en dos paneles; en móvil usa un flujo `lista → detalle → Back`, mueve el foco al encabezado al abrir y lo devuelve a la inquiry seleccionada al regresar. La lista separa `New` y `Read`; el detalle reúne hechos, notas, moodboard y acciones `mailto:`/`tel:`. Cambiar el estado es una operación optimista que revierte y explica el fallo si Neon rechaza la actualización. Resend no es requisito para revisar o responder manualmente.
