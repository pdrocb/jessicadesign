...
Look Book albums follow the same discipline: a cover photograph and a two-ended caption 14px below, inset 8px from both image edges. The project title aligns left; the right side shows `Location`, falling back to `Venue` only when location is absent. Both labels remain on one line and truncate only when the available width cannot contain them. Type remains 9px/`.14em` on mobile, 10px/`.16em` from tablet and `.18em` on wide screens. Captions sit in `ink-subtle` so the photographs stay the loudest thing in the section.
...
The gallery moves through the same structural breakpoints: one column on mobile, six columns on tablet and twelve on desktop, always with a 16px gap. Desktop runs landscape·portrait (`8/4`), a balanced landscape pair (`6/6`) cropped to 5:4, then three squares (`4/4/4`). Tablet and mobile preserve all selected projects; no active project disappears at an intermediate breakpoint.

The seven shapes belong to positions in the composition—wide, tall, two paired frames and three squares—rather than to a second per-project order. Selecting another project places its main cover into the available slot while preserving the mosaic.
...
The home version is a curated doorway, not the complete archive. It shows up to seven published projects marked `Show on Home`, always in the same order as the complete Look Book. The first gallery photograph is the shared cover in both destinations, and every cover links directly to its project anchor. The closing action always opens the full page. With no selected projects, the complete Home section disappears.
...
`coverImageId` is the single project cover for both `/look-book` and Home. Choosing another principal photograph moves it to position one and updates both destinations. The CMS data contract records intrinsic dimensions and supports `focalPoint` plus `cropTolerance: none | soft`; previews default to the soft editorial crop, while `none` restores the exact source ratio when an image cannot tolerate cropping. Each chapter closes with the secondary `View Full Gallery` action and a photograph count.
...
The two inquiry emails reuse the public system without turning an inbox into a miniature webpage. Both sit on `cream` with a 640px `paper` table, square corners, no shadow and warm hairlines. Cormorant/Georgia carries names and acknowledgements; Karla/Arial carries every operational detail. `Sage` is reserved for the internal arrival banner; `petal` holds the submitted vision in the operational template.

The internal template optimizes retrieval: name, received time, celebration and date lead; every submitted field follows in a scan-friendly table; email and phone remain actionable, and Pinterest appears as an actionable link only when supplied. The client template is a personal editorial note, not a miniature landing page: the monogram is its only masthead, the recipient’s first name opens one continuous paper plane, the approved paragraph flows directly into a first-name signature, and a dark minimal footer carries location and contact links without repeating the brand name. It does not repeat the inquiry or introduce a process sequence. Neither template claims a proposal, quote, contract, event-planning responsibility or response window more specific than “within a few days.” Both include a plain-text equivalent and preserve the shared responsive email shell below 620px; footer links stack at the narrow breakpoint.
