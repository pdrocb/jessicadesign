...
El Look Book es una sola página editorial en `/look-book`, no una colección de páginas individuales. Cada proyecto muestra como portada la primera fotografía de su galería, contexto y una selección compacta de hasta cinco fotografías en el orden publicado. Las selecciones conservan ese orden en todas las resoluciones y usan marcos de proporción editorial con recorte suave: tres columnas como máximo en desktop y dos columnas proporcionales —no fijas— en tablet y mobile. Cualquier fotografía abre un visor de pantalla completa en su posición correspondiente; el CTA `View Full Gallery` abre el proyecto completo desde la primera fotografía.

El orden editorial es dato, no consecuencia del nombre del archivo: cada proyecto y cada fotografía tienen una posición explícita. La posición 1 es siempre la portada; al elegir otra portada, el CMS la pasa a esa posición y reacomoda el resto. Home muestra hasta siete proyectos publicados que tengan activo `Show on Home`, conserva exactamente el mismo orden del Look Book y usa la misma portada principal. Cambiar el orden general en el CMS cambia ambos destinos; el toggle únicamente filtra, no crea un segundo orden editorial. Cada entrada enlaza al capítulo `/look-book#slug`, muestra el título a la izquierda y, a la derecha, solo `Location` o `Venue` como respaldo cuando no existe ubicación. Ambos textos permanecen en una sola línea. Si no hay proyectos seleccionados, la sección completa no se publica.
